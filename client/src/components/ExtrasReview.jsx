...
/**
 * The review screen for a one-off addition an assistant proposed — a meal
 * time, a leisure or family-time slot — none of which are fixed
 * commitments, so each week's timetable includes its own suggestions.
 * Same review-before-write shape as everything else: tick what you want,
 * adjust anything that needs it, then save. Saving adds each one as a
 * one-off commitment scoped to that single day, so it never disturbs any
 * other week.
 */
export function ExtrasReview({ rows, onChange, onSave, onDismiss, saving, noun, description }) {
    ...
    <Card className="mt-4 p-4">
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <TextInput
                ...
                className="w-32"
                aria-label={`Name for this ${noun} on ${row.scheduled_date}`}
              />
      ...
      <div className="mt-4 flex items-center justify-between gap-2">
        ...
        <Button variant="primary" onClick={() => onSave(rows)} disabled={saving || included.length === 0}>
          {saving ? 'Saving…' : `Save ${included.length} ${noun}${included.length === 1 ? '' : 's'}`}
        </Button>
      </div>
    </Card>
  );
